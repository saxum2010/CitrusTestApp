					...
					success: function(res){
			            ...
			            if(res.needUpdate=='Y'){
							...
							if(!MobileUser.IsAuthorized){
								var loStatus = MobileUser.GetStorage('needLogin');
								if(loStatus!=undefined){
									$('#needLogin').remove();
								}else{
									setTimeout(function(){
									$('#needLogin').html('<div class="needLogin_bg"></div><div class="needLoginContent"><div class="needLoginLogo"><img src="img/png/logo.png"></div><div class="needLoginTitle">Представьтесь</div><div class="needLoginText">Авторизуйтесь сейчас, получите <br /> доступ к бонусному счету и другим <br /> функциям приложения</div><br /><span class="needLoginEnterNumberText">Введите номер телефона</span><input type="tel" id="needLoginEnterNumber" maxlength="13" autocomplete="off"  value="+380" onkeyup="MobileUser.VerifyPhoneInput(this,\'#needLoginButton\')"><button id="needLoginButton" class="green_btn ui-btn ui-corner-all"><i class="c_icon c_need_update c_ibtn"></i>	 Авторизоваться</button><span class="needNoLogin">Спасибо. Продолжаю использовать приложение без <br />дополнительных функций</span></div>').show();
									}, 60000);
								}
							}
			            }
